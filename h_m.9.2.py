...
# *args: Union[int, float] — функция принимает любое количество аргументов,
# каждый из которых может быть int или float.
# -> float — функция возвращает число с плавающей точкой (float),
# потому что результат может быть нецелым и мы применяем round().
...
def difference(*args: Union[int, float]) -> float:
